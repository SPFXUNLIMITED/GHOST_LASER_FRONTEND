...
<body class="bg-zinc-950 text-white font-sans antialiased {{ $bodyClass }}">
    ...
    @php
        ...
        $customerLoggedIn  = (bool) $customer;
        $customerFirstName = $customerLoggedIn ? trim((string) $customer->first_name) : '';
        $customerLastName  = $customerLoggedIn ? trim((string) $customer->last_name) : '';
        $customerEmail     = $customerLoggedIn ? trim((string) $customer->email) : '';
                ...
                <div class="flex-1 min-w-0 flex items-center justify-end gap-3">
                    ...
                    <div class="relative" id="header-user-menu">
                        <button
                            ...
                            aria-expanded="false"
                        >
                            <span class="header-user-avatar">{{ $customerInitials }}</span>
                            <span class="max-w-[9rem] truncate text-sm font-medium text-zinc-200">{{ $customerFirstName !== '' ? $customerFirstName : 'Customer' }}</span>
                        ...
                        >
                            <div class="px-2.5 py-2">
                                <p class="text-sm font-semibold text-white truncate">{{ $customerFullName }}</p>
                                <p class="mt-0.5 text-xs text-zinc-400 truncate">{{ $customerEmail }}</p>
                            </div>
