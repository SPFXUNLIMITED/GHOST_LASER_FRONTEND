...
<section class="pb-24 lg:pb-32 bg-zinc-950">
    <div class="max-w-3xl mx-auto px-6">
        ...
        <?php
            ...
            $distanceCustomerAddress = implode(', ', array_filter([
                $booking['address'] ?? '',
                $booking['city']    ?? '',
                $booking['state']   ?? '',
                $booking['zip']     ?? '',
            ]));
            $distanceResult = calculateDrivingDistanceMiles(
                (string) ($travelSettings['base_location'] ?? ''),
                $distanceCustomerAddress,
                $googleMapsApiKey
            );
        ...
        <div class="space-y-6 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6 sm:p-8 glow-box">
            ...
            <div id="step-2-booking-shell">
                ...
                <form id="step-2-booking-form" class="space-y-4 mt-4" novalidate>
                    ...
                    <input type="hidden" name="booking_source" value="Internal">
                    ...
                    <div class="rounded-lg border border-red-500/40 bg-red-950/40 px-4 py-3 text-sm text-red-300">
                        Travel will be calculated at the time of booking — <?= h(travelDistanceErrorMessage($travelDistanceError)) ?>
                    </div>
                    <?php endif; ?>
    ...
    if (stepTwoForm && bookingPayload) {
        ...
        stepTwoForm.addEventListener('submit', async (event) => {
            ...
            const problemSections    = [
                ...
                travelDistanceError
                    ? `Travel fee: unable to calculate — will be determined at time of booking (${travelDistanceError}) — contact us for a quote`
